CLASS ycl_dmo_unit_test DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  FOR TESTING
...
CLASS ycl_dmo_unit_test IMPLEMENTATION.
  METHOD class_setup.
    DATA:
  ...
  METHOD create_new_entry.
    DATA: lt_new_partner TYPE TABLE FOR CREATE ydmo_partner_i.
    ...
    COMMIT ENTITIES RESPONSE OF ydmo_partner_i
    REPORTED DATA(lt_reported)
    FAILED   DATA(lt_failed).

    cl_abap_unit_assert=>assert_initial( lt_reported-PartnerBDI ).
    cl_abap_unit_assert=>assert_initial( lt_failed-PartnerBDI ).
  ...
  METHOD fill_empty_streets.
    DATA: lt_fill_streets TYPE TABLE FOR ACTION IMPORT ydmo_partner_i~fillEmptyStreets.

    lt_fill_streets = VALUE #( ( PartnerNumber = '2000000001' ) ).

    MODIFY ENTITIES OF ydmo_partner_i
      ENTITY PartnerBDI EXECUTE fillEmptyStreets FROM lt_fill_streets
      MAPPED DATA(ls_mapped)
      FAILED DATA(ls_failed)
      REPORTED DATA(ls_reported).

    COMMIT ENTITIES
      RESPONSE OF ydmo_partner_i
  ...
  METHOD clear_empty_streets.
    DATA:
  lt_clear_streets TYPE TABLE FOR ACTION IMPORT ydmo_partner_i~clearAllEmptyStreets.

    INSERT INITIAL LINE INTO TABLE lt_clear_streets.

    MODIFY ENTITIES OF ydmo_partner_i
      ENTITY PartnerBDI EXECUTE clearAllEmptyStreets FROM lt_clear_streets
      MAPPED DATA(ls_mapped)
      FAILED DATA(ls_failed)
      REPORTED DATA(ls_reported).
    ...
    SELECT FROM ydmo_partner_i
      FIELDS PartnerNumber
      WHERE street = 'EMPTY'
